...
var JoiForm = React.createClass({
    ...
    componentDidMount: function componentDidMount() {
        ...
        if (this.props.schema) {
            this.props.schema.forEach(function (fieldSchema) {
                ...
                var fieldName = meta.name || _this._camelize(fieldSchema._flags.label);
                // var fieldName = meta.name || this._camelize(fieldSchema._options.language.label);
    ...
    getInitialState: function getInitialState() {
        ...
        if (this.props.schema) {
            this.props.schema.forEach(function (fieldSchema) {
                ...
                var meta = fieldSchema._meta[0] || fieldSchema._meta;
                var name = meta.name || _this2._camelize(fieldSchema._flags.label);
    ...
    componentWillReceiveProps: function componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.schema) {
            nextProps.schema.forEach(function (fieldSchema) {
                var meta = fieldSchema._meta[0] || fieldSchema._meta;
                var fieldName = meta.name || _this3._camelize(fieldSchema._flags.label);
                schema[fieldName] = fieldSchema;
            });
        }
    ...
    render: function render() {
        if (this.props.inline) {
            return React.createElement(
                'div',
                null,
                this.props.children || React.createElement(FormSection, null)
            );
        }
    ...
    submit: function submit(e) {
        ...
        Joi.validate(this.state.values, this.state.schema, _extends({ abortEarly: false }, this.props.validateOpts), function (err, value) {
            if (err) {
                var formErrors = {};
    ...
    __onChange: function __onChange(e, values) {
        ...
        Object.keys(values).forEach(function (k) {
            var isArrayKey = k.match(/([\w-]+)\[(\d+)\]/); // matches 'fieldname[1]' like keys
            if (isArrayKey) {
    ...
    __onSelect2Search: function __onSelect2Search(e, change) {
